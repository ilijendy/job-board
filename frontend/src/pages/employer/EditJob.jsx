...
export default function EditJob() {
    ...
    useEffect(() => {
        ...
        }).catch(() => setError('Could not load job details')).finally(() => setFetching(false));
    }, [id]);
    ...
    const handleSubmit = async e => {
        ...
        try {
            await api.put(`/employer/job/${id}`, form);
    ...
    if (fetching) return <div className="min-h-screen flex items-center justify-center"><div className="h-8 w-8 border-4 border-brand-600 border-t-transparent rounded-full animate-spin" /></div>;
    ...
    return (
        <div className="min-h-screen flex flex-col bg-slate-50">
            ...
            <main className="flex-1 mx-auto w-full max-w-3xl px-4 sm:px-6 py-10">
                <div className="mb-8">
                    <p className="text-sm font-semibold uppercase tracking-widest text-brand-700">Edit Listing</p>
                    <h1 className="mt-1 text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">Update Job Posting</h1>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border border-slate-200/80 bg-white p-6 sm:p-8 shadow-card">
                    ...
                    <div>
                        ...
                        <textarea name="description" value={form.description} onChange={handleChange} rows={5} className={inputClass} required />
                    </div>
                    <div>
